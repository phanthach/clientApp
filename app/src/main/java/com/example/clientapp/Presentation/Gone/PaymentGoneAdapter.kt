...
class PaymentGoneAdapter(private val mList: MutableList<Payment>, private val itemListener: ItemListener): RecyclerView.Adapter<PaymentGoneAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding:ItemHistoryBinding, private val itemListener: ItemListener) : RecyclerView.ViewHolder(binding.root){
        fun bind(item: Payment, position: Int){
            ...
            val time = convertTimeModern(item.paymentTime)
            binding.tvCancleTimeTitle.text = "Thanh toán lúc: "
            if(item.paymentStatus==1){
                binding.status.text = "Đã thanh toán"
                binding.status.setTextColor(binding.root.resources.getColor(R.color.colorAccent))
            }
            else{
                binding.status.text = "Đã sử dụng"
