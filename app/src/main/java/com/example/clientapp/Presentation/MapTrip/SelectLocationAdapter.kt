...
class SelectLocationAdapter(private var listLocation:MutableList<Location>, private val itemListener: ItemListener):RecyclerView.Adapter<SelectLocationAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding: ItemPickLocationBinding, private val itemListener: ItemListener):RecyclerView.ViewHolder(binding.root) {
        fun bind(location: String, type:Int, position: Int) {
            binding.location.text = location
            binding.distance.visibility = ViewGroup.GONE
            if(type == 1){
                binding.imagePoints.setImageResource(com.example.clientapp.R.drawable.lettera)
            }
            else{
                binding.imagePoints.setImageResource(com.example.clientapp.R.drawable.letterb)
            }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        if (listLocation.isNotEmpty()){
            holder.bind(listLocation[position].nameLocation!!,listLocation[position].locationType!!, position)
        }
    }
